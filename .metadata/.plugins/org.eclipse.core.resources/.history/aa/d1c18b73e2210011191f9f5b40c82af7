...
/** A simple means of rejecting invalid sentences
 *
 * Only returns success when all of the steps leading up to well parsed
 * data are fulfilled
 *
 * @return 0 if successful
 */
int WindSense::processInternalNMEA() {
	if (validateInternalNMEA()) {
		resetInternalNMEA();
		return 1;
	}
	splitInternalNMEA();
	if (parseInternalNMEA()) {
		resetInternalNMEA();
		return 1;
	}
	resetInternalNMEA();
